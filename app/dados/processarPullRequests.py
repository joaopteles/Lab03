...
def importarPullRequest(repo, repo_index, pr_first, token, page_info, total, max_prs):
    sucess = True
    ...
    while page_info['hasNextPage']:
        try:
            ...
            sm.write_pull_file(repo['nome'], [pullRquestData(x, repo) for x in new_prs if pullRquestValido(x)])
...
def iniciar(repo_limit, pr_first, token):
    ...
    for index in range(repo_index, repo_limit):
        ...
        sucess = importarPullRequest(repo, repo_index, pr_first, token, page_info, total, max_prs)
        if not sucess:
            break
        page_info['endCursor'] = ""
