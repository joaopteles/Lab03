import app.csv_manager.util as sm
import math

base_path_pull = '{}/pull_requests'.format('tmp')

def write_pull_state(end_cursor, has_next_page, repo_index, total):
    sm.write_file('{}/state.csv'.format(base_path_pull), [{ "end_cursor": end_cursor, "has_next_page": has_next_page, "total": total, "repo_index": repo_index}])
